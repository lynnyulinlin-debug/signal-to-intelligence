...
# ── Figure 1: CNN local receptive field vs ViT global self-attention ──────────
...
# Draw 5x5 pixel grid (light gray)
for i in range(6):
    ax.plot([0, 5], [i, i], color='#cccccc', linewidth=0.8)
    ax.plot([i, i], [0, 5], color='#cccccc', linewidth=0.8)

# Fill all cells with very light blue
for r in range(5):
    for c in range(5):
        rect = mpatches.FancyBboxPatch((c + 0.02, r + 0.02), 0.96, 0.96,
                                       boxstyle='square,pad=0',
                                       facecolor='#e3f2fd', edgecolor='none')
        ax.add_patch(rect)

# Highlight 3x3 receptive field (center)
for r in range(1, 4):
    for c in range(1, 4):
        rect = mpatches.FancyBboxPatch((c + 0.02, r + 0.02), 0.96, 0.96,
                                       boxstyle='square,pad=0',
                                       facecolor='#ef9a9a', edgecolor='none')
        ax.add_patch(rect)

border = mpatches.Rectangle((1, 1), 3, 3, linewidth=2.5,
                             edgecolor='#c62828', facecolor='none')
ax.add_patch(border)
ax.text(2.5, 2.5, '3×3\nkernel', ha='center', va='center',
        fontsize=10, fontweight='bold', color='#b71c1c')

ax.annotate('Only 9 pixels\nvisible at once', xy=(4.0, 1.5), xytext=(3.3, 0.2),
            fontsize=9, ha='center', color='#c62828',
            arrowprops=dict(arrowstyle='->', color='#c62828', lw=1.2))

ax.set_xlabel('Pixels', fontsize=10, labelpad=4)
ax.text(2.5, -0.35, 'Each output sees only a small local region',
...
for r in range(4):
    for c in range(4):
        ...
        rect = mpatches.FancyBboxPatch((c + 0.04, r + 0.04), 0.92, 0.92,
                                       boxstyle='round,pad=0.03',
                                       facecolor=color, edgecolor='white', linewidth=1.5)
        ax.add_patch(rect)
        ax.text(c + 0.5, r + 0.5, f'P{r*4+c+1}', ha='center', va='center',
                fontsize=7, color='white', fontweight='bold')

# Draw attention lines from patch P6 (row 1, col 1) to all others
qr, qc = 1, 1  # query patch
qx, qy = qc + 0.5, qr + 0.5
for r in range(4):
    for c in range(4):
        if r == qr and c == qc:
            continue
        tx, ty = c + 0.5, r + 0.5
        ax.annotate('', xy=(tx, ty), xytext=(qx, qy),
                    arrowprops=dict(arrowstyle='->', color='#1565c0',
                                   alpha=0.35, lw=0.9))

# Highlight query patch
rect = mpatches.FancyBboxPatch((qc + 0.04, qr + 0.04), 0.92, 0.92,
                               boxstyle='round,pad=0.03',
                               facecolor='#e53935', edgecolor='#b71c1c', linewidth=2.5)
ax.add_patch(rect)
ax.text(qc + 0.5, qr + 0.5, 'Q', ha='center', va='center',
        fontsize=11, fontweight='bold', color='white')

ax.text(2.0, -0.45, 'Each patch attends to ALL other patches',
        ha='center', fontsize=9, color='#1565c0', style='italic')
ax.set_xlabel('16×16 px patches', fontsize=10, labelpad=4)

plt.tight_layout()
plt.savefig('assets/ch07_vit_cnn_comparison.png', dpi=100, bbox_inches='tight')
plt.close()
print("Saved assets/ch07_vit_cnn_comparison.png")

# ── Figure 2: Temperature parameter effect on softmax distribution ─────────────
...
# Similarity scores: 1 correct match, 4 negatives
...
for i, (tau, subtitle, note, nc) in enumerate(zip(taus, subtitles, notes, note_colors)):
    ...
    ax.text(0.5, 0.97, note, transform=ax.transAxes,
            ha='center', va='top', fontsize=8.5, color=nc,
            bbox=dict(boxstyle='round,pad=0.3', facecolor='#fffde7', alpha=0.9))

plt.tight_layout()
plt.savefig('assets/ch07_temperature_effect.png', dpi=100, bbox_inches='tight')
plt.close()
print("Saved assets/ch07_temperature_effect.png")
