...
池化不是深度学习的核心魔法，但它在早期 CNN 里常被用来压缩空间尺寸、扩大感受野并减少计算。
...
- 可学习的参数
- 网络自动学习适合当前任务的卷积核
- 与 [1.6 信号检测 - 匹配滤波器](../01_dsp/06_signal_detection.md#_3-匹配滤波器-matched-filter) 的思想相似：都是通过卷积提取对任务有用的信息

### 启示

**CNN可以理解为学习一组适合任务的滤波器。**

换句话说，CNN 不是“替代卷积”，而是把卷积从手工设计的信号处理步骤，变成了端到端可学习的表示模块。
...
这条“从局部到整体”的路径，就是 CNN 在视觉里最重要的归纳偏置。
...
也就是说，CNN 的价值不只是“做分类器”，而是为检测、分割等更复杂任务提供通用视觉表征。
...
这正好解释了为什么第 3 章后面会继续从视觉走到序列，再走到 Transformer。
