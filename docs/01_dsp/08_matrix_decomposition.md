...
在深度学习出现之前，PCA、SVD 以及基于滤波器组的手工特征（如 MFCC、HOG、SIFT）是信号与图像处理的主流特征提取方式。它们的核心思想是：用少数几个方向或基函数来紧凑地表示信号的主要结构，同时丢弃噪声和冗余。理解这些方法有助于理解为什么后续章节中的表示学习如此有效，详见 [第3章深度学习基础](../03_deep_learning_fast/02_cnn_essence.md)。
