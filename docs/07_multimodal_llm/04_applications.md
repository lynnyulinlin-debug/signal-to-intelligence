...
CIDEr 提升最大，说明 Qwen2.5-VL 在描述细节的特异性上有显著优势。
...
**TextVQA 为什么难？** 需要识别图像中的文字（OCR）再推理。LLaVA-1.5 在 224×224 分辨率下文字像素太少，Qwen2.5-VL 的高分辨率支持使 TextVQA 提升达 +12.9%——这正是高分辨率的核心价值（见 7.3）。
...
- **VQA** 需要跨模态注意力对齐图像内容与问题语义；高分辨率对 TextVQA 尤为关键（+12.9%）
- **图像搜索** 在 CLIP 对齐的高维空间用余弦相似度匹配，支持零样本检索而无需手工标注
- 三类应用的性能提升都直接受益于 7.3 讨论的高分辨率处理能力
