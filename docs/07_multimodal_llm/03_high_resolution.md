...
根据奈奎斯特采样定理，要保留图像中的高频细节（如文字边缘、细线），采样率必须至少是该细节频率的2倍。
...
分块处理（512×512，重叠50%）：
  ...
  关键：非重叠区域信息完整保留，重叠区域产生冗余（用于边界连续性）
  注意：重叠会使总处理像素增加，不是"信息量增加"
```
...
**实际效果：** TextVQA 从 LLaVA-1.5 的 74.2 提升到 87.1（+12.9），主要受益于文档场景下的高分辨率支持。
