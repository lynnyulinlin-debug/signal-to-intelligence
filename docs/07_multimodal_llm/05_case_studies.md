...
下面的数字用于说明高分辨率模型在文档场景中的典型收益方向，具体结果取决于数据集、OCR/预处理、提示词和模型版本。
...
表格识别通常受益最大，因为表格需要同时理解空间布局和文字内容——高分辨率的收益最直接体现在这里。
...
**Chain-of-Thought 在图表分析中的价值：** 让模型先描述图表结构再分析数据，通常比直接问"分析这个图表"更稳——因为结构描述步骤会迫使模型先定位坐标轴，减少后续推理基于错误数值的风险。
### 性能分析
...
### 为什么以 Qwen2.5-VL 为代表的中文多模态模型在中文场景更有优势
...
以 Qwen2.5-VL 为代表的中文多模态模型使用视觉编码器 + 中英双语对齐训练
  ↓
中文理解质量接近英文水平
```

**为什么小语种可能弱于中文/英文？** 视觉文字形态、tokenizer 覆盖率和预训练数据量都会影响多语言图文对齐质量。
...
这个示意结果体现了语言数据分布对多模态理解的影响；实际排序需要按具体模型和评测集确认。
...
*图7.5：三个实战案例性能对比示意——文档理解、图表分析、多语言应用在 LLaVA-1.5 与 Qwen2.5-VL 上的关键指标变化。*
...
- **文档理解** 的核心瓶颈是分辨率（字符级精度）+ 结构化输出（JSON + schema 校验）；表格识别通常最受益于高分辨率
- **图表分析** 需要视觉感知 + 数值理解 + 语义推理三步；先描述结构再分析数据，通常比直接分析更稳
- **多语言应用** 的性能差异来自预训练数据分布、tokenizer 覆盖和图文对齐数据质量
