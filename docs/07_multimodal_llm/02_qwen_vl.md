...
图像分辨率与最大可编码信息量上界的关系（假设每像素独立均匀分布，实际图像远低于此值）：
```
信息量上界 = log₂(分辨率²) = 2 × log₂(分辨率)

224×224：  上界 ≈ 2 × log₂(224) ≈ 15.8 bits
1024×1024：上界 = 2 × log₂(1024) = 20 bits

上界差异：约 4.2 bits。高分辨率的真正价值不在信息量，而在于
细节像素的保留——文字、坐标轴刻度等只有在足够多的像素下才可识别。
```
