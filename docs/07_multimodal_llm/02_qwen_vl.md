...
1. **分辨率限制**
   - 早期版本固定224×224，LLaVA-1.5提升到336×336，仍低于Qwen2.5-VL的1024×1024
   - 丢失图像细节
   - 无法处理文档、图表
...
- 自注意力复杂度：O((D_img + D_text)²)（省略隐层维度 d，实际为 O((N_img+N_text)²·d)）
- 问题：维度增加，复杂度平方增长
```
...
图像分辨率与最大可编码信息量的关系（理论上界，假设每像素独立）：
```
最大信息量上界 = log₂(分辨率²) = 2 × log₂(分辨率)

224×224：  上界 = 2 × log₂(224) ≈ 15.8 bits/像素维度
1024×1024：上界 = 2 × log₂(1024) = 20 bits/像素维度

理论差异：约 4.2 bits——但实际图像内容远低于此上界，
高分辨率的真正价值在于保留细节像素（文字、坐标轴刻度等）。
...
类似适配器的瓶颈设计（并非 LLaMA-Adapter 标准适配器），通过低维中间层降低参数量：
```
原始投影层：
  参数量 = 4096 × 768 ≈ 3.1M
...
| 场景 | LLaVA-1.5 | Qwen2.5-VL | 量化提升（代表基准） |
|------|-----------|-----------|--------------------|
| 视觉问答 | ✓ 80.0 | ✓✓ 84.3 | +4.3 pts (VQAv2) |
| 图像描述 | ✓ | ✓✓ | CIDEr +16.4 |
| 文档理解 | ✗ | ✓✓ 90.1 | 新能力 (DocVQA) |
| 图表分析 | ✗ | ✓✓ | 新能力 |
| 中文应用 | ✗ | ✓✓ | 新能力 |
| 多图像对比 | ✗ 52.3 | ✓ 68.7 | +16.4 pts (Mantis) |
