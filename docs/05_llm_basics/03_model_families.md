...
**核心问题：** 现在有哪些主流 LLM？它们是怎么演化来的？开源和闭源模型有什么区别？
...
1. **理解能力来源**：知道某个能力是哪个版本引入的，能帮助理解"为什么这样设计"
2. **选型依据**：微调、部署、API 调用时，需要知道各模型的特点和适用场景

---

## GPT 系列（OpenAI）

GPT 系列是现代 LLM 的起点，定义了"预训练 + 微调"的范式。
...
LLaMA 系列是开源 LLM 生态的基石，推动了整个开源社区的发展。
...
**LLaMA 的生态意义：** LLaMA 发布后，社区基于它微调出了数百个专用模型（Alpaca、Vicuna、WizardLM 等），形成了繁荣的开源生态。

---

## Qwen 系列（阿里巴巴）

Qwen（通义千问）是中文能力最强的开源模型系列之一。
...
**Qwen 的特点：** 中文能力强、覆盖规模全、专用模型丰富，是中文场景微调的首选。
...
- 快速验证、追求最强能力 → 闭源 API
- 中文场景 → Qwen 系列
- 商业部署、许可证要求严格 → Mistral（Apache 2.0）
...
- **GPT 系列** 定义了 LLM 的发展范式：规模 → 对齐 → 多模态 → 推理
- **LLaMA 系列** 是开源生态的基石，推动了社区微调和应用开发
- **Qwen 系列** 是中文场景的首选，覆盖从 0.5B 到 72B 的全规模
- **开源 vs 闭源** 的选择取决于数据隐私、定制需求、成本和能力要求
- 了解模型家族是微调选型的基础——下一节将介绍如何根据任务选择合适的基础模型
