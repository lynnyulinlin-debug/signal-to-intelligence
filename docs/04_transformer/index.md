...
本章不是只解释注意力公式，而是要说明：**为什么 self-attention、位置编码、残差连接和因果掩码组合在一起后，Transformer 会成为现代 LLM 的主干架构。**

这一章既承接前面关于矩阵分解、深度学习和序列建模的内容，也是进入后续 LLM 与多模态章节前最关键的一步。

本章重点关注：
- 自注意力如何在一次前向传播里动态聚合全局信息
- 多头注意力为什么不仅是“并行重复”，而是表达能力升级
- 为什么位置必须显式注入，以及它与傅里叶思想的联系
- 残差连接、LayerNorm 和 FFN 为什么让深层训练更稳定
- 为什么 causal mask 和 decoder-only 结构会成为现代 LLM 的主流选择
...
Transformer Block：Attention + FFN + Residual + Norm
    ↓
Decoder-only Transformer：用因果掩码做自回归生成
    ↓
LLM：大规模预训练的生成式序列模型
```
...
### 原理理解部分
1. Self-Attention 为什么比 RNN 更适合长距离依赖和并行训练？
...
7. encoder-only、encoder-decoder、decoder-only 在训练目标和推理方式上有什么差异？
8. 为什么现代 LLM 大多采用 decoder-only Transformer？

### 应用实践部分
9. 如何用代码实验直观理解注意力权重、mask 和位置编码？
10. 第 4 章的哪些设计直接决定了后续 LLM 与多模态章节的实现方式？
...
A: 因为自注意力允许任意两个位置直接交互，而 RNN 必须通过逐步递推传递信息，路径更长，也更难并行训练。
...
A: 因为它天然适合 next-token prediction，自回归训练目标简单统一，且在训练和推理阶段都更容易扩展到大规模文本生成任务。
