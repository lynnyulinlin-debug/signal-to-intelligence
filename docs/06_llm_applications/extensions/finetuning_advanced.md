...
| 场景 | 推荐工具 |
|------|---------|
| 快速验证，不想写代码 | LLaMA-Factory（Web UI） |
| 消费级 GPU（RTX 3090/4090） | Unsloth + QLoRA |
| 需要自定义训练逻辑 | PEFT + Transformers |
| 大规模分布式训练 | DeepSpeed + PEFT |
...
定量评估：在保留的测试集上计算任务指标（准确率、BLEU、ROUGE 等）。
