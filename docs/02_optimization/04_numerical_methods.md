...
**核心问题：** 为什么某些优化算法在实践中会失败？如何确保数值计算的稳定性？

---

## 为什么需要理解数值方法

在理论上完美的算法在实现时可能会失败。这不是算法的问题，而是**数值计算的问题**。

### 实际例子
在训练神经网络时，下面这些情况都很常见：

- 损失函数前几步看起来正常，但很快变成 `NaN`
- 梯度在深层网络里逐层变小，训练几乎不动
- 不同实现的“同一个公式”因为浮点误差而得到不同结果

这些问题往往不是模型结构本身导致的，而是数值精度、条件数和实现方式共同造成的。
---
...
一个算法是**数值稳定的**，如果小的输入误差不会导致大的输出误差。
...
梯度检查的意义不在于替代反向传播，而在于帮助定位实现错误。
如果数值梯度和解析梯度差很大，优先怀疑代码实现，而不是先怀疑模型理论。
...
- **第4-8章（Transformer / LLM）**：注意力分数缩放、LayerNorm、混合精度和学习率调度都和这里的原则一致
