...
决策树通过递归分割特征空间来做预测；随机森林通过集成多棵树来降低过拟合。
...
**思想：** 通过 Bootstrap 采样和特征随机性，训练多个决策树，再用投票或平均完成预测。
...
这个类比只强调"多个子模型/子空间结果再融合"的思想。随机森林是多个树的投票或平均，多头注意力是在同一个端到端模型中学习多个表示子空间，两者不是同一种机制。
...
1. **可扩展性**
   - 随机森林不适合端到端训练数十亿级连续参数
   - Transformer可以借助小批量训练、加速器和分布式系统扩展
...
虽然LLM不直接使用随机森林，但集成学习中的"多路结果融合"思想可以帮助理解 LLM 中的一些设计：
...
2. **为什么多头注意力不是随机森林**
   ...
   - 多头注意力：可学习的融合权重
   - 可以根据任务自动调整不同头的重要性
   - 多头之间共享同一模型的训练目标，而随机森林中的树通常相对独立训练
...
1. **可扩展性**
   - 随机森林：不适合端到端训练数十亿级连续参数
   - LLM：通过 Transformer 架构、加速器和分布式训练扩展到大规模参数
...
4. **端到端优化**
   - 随机森林：贪心分割，局部最优
   - LLM：通过反向传播端到端优化，但不保证找到全局最优

5. **表达能力**
   - 随机森林：受限于树的结构
   - LLM：可以学习复杂的语言模式和任务表示
...
**下一章：** 阅读 [第3章：深度学习快速入门](../03_deep_learning_fast/README.md)
