...
**核心问题：** 优化算法如何影响LLM的训练？为什么需要学习传统机器学习？
...
这里先讲优化器，不是为了堆概念，而是为了说明后面所有模型都共享同一件事：**训练本质上是一个带约束的数值优化过程**。
线性回归、逻辑回归、SVM 和树模型看起来不一样，但它们都离不开目标函数、更新规则和数值实现。
先理解优化器，再看传统 ML，读者更容易把“模型是什么”和“它怎么被训练出来”分开看清楚。
