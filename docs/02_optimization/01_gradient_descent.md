...
- **非凸优化**：神经网络的损失函数通常是非凸的，梯度下降类方法是最常用、最可扩展的训练方法
...
**Q: 为什么梯度下降会陷入局部最优？**
A: 对非凸问题，梯度下降不能保证找到全局最优。深度网络中更常见的困难包括鞍点、平坦区域和病态曲率；过参数化模型中的许多可达解效果不错，但这不是所有问题都成立的定理。
...
**Q: 为什么需要反向传播？**
A: 反向传播用反向模式自动微分一次性计算所有参数的梯度，成本通常和前向传播同量级。相比逐个参数做数值差分，它使大规模神经网络训练成为可能。

**Q: 梯度下降和随机梯度下降有什么区别？**
A: 梯度下降用所有数据计算梯度，收敛稳定但计算慢。随机梯度下降用一个样本计算梯度，计算快但梯度有噪音。小批量梯度下降是两者的折中。

**Q: 为什么神经网络需要梯度下降？**
A: 神经网络的损失函数通常是非凸的，没有通用闭式解。梯度下降类方法能和反向传播高效结合，因此成为深度学习训练的主流方法。
